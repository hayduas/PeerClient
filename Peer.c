...
/*
 * A replacement to the 'getch()' function found on the windows-based "conio.h" library. This function is
...
/*
 * Sends a TCP message to a specific socket.
 */
...
/*
 * Sends a TCP message to a specific SOCKET - IP and PORT.
 */
...
/*
 * Sends a message to the peer with message type
 * 0-TAKE
 * 1-eror
 * 2-FBAD
 * 3-GOOD
...
/*
 * Preforms a sequence for quitting.
...
/*
 * Displays error message (string) on screen, and performs quitSequence.
 */
...
/*
 * Restores chunks of files in the ./shared dir to original files in ./completed dir.
 */
...
/* 
 * Checks if we have the chunk name in our file list. Returns int:
 * -1 - We already got this chunk / chunk name isn't valid
...
/*
 * Reads peer response - blocking if no data
 */
		 	 ...
		 	 if (strcmp(msgType,"FGET")==0)
		 		...
		 		//----NOCR Message=------------------------------------------------------
		 		 if (strcmp(msgType,"NOCR")==0)
		 		 {
		 			...
		 			printf("please upload some chunks to this peer in order to increase your credit\n\r");
		 			return 3;
		 		 }
		 		 else
		 		//----TAKE  Message=------------------------------------------------------
...
/*
 * Main func. for analysing response after sending messages to SERVER.
...
int analyzeServerResponse(int s)
{
	char buf[BUF_SIZE];
	int bytes,i;
	char msgType[5];   // Will hold message type received
	char *tmp=0;	   // For tokenizer usage
...
/*
 * Sends a search query to server, and displays file-list returned
 */
...
/*
 * Sends a query for who is online to server, and displays a list of users online returned.
 */
void whoQuery(int s)
{
	char message[4096]; // Message to send
...
/*
 * Sends a list of files (chunks) from the ./shared dir to be shared on server.
 */
void sendList(int s)
{
	...
	if (n >= 0)
	{
		...
		{
			strcat(message,"#");
			strcat(message,eps[cnt]->d_name);  // Add current file to message
		}
	}
	else
...
/*
 * Performs a login to the server using an encrypted password.
 */
...
/*
 * Creates chunks of files in ./Completed dir with the size of CHUNK_SIZE
 */
...
void startPeerCommunication(int s)
{
	...
	while(1) // analyzePeerResponse will block if there is no data
	{
...
void listenToPeers()
{
    ...
    while (1)
	{
		sa = accept(s1, 0, 0);		/* blocks for connection request */
...
/*
 * Opens UDP port for UDP server and listens to traffic
 */
...
void sendMessageToPeerUdp(int ud)
{
	...
	printf("please enter the name of the user that you want to chat with\n");
	req=getString(4,1,"");
	strcat(buf,"MSEG");
	strcat(buf,req);
	strcat(buf,"says:");
